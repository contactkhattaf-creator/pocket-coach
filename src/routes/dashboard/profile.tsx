...
function FinancialProfilePage() {
  ...
  async function analyzeProfile() {
    ...
    // Category analysis for profile detection
    ...
    const fdsScore = Math.round(
      (savingsRatio * 40) + // 40% weight on savings
      (goalScore * 30) + // 30% on goals
      (Math.min(transactions.length / 50, 1) * 20) + // 20% on activity
      (Math.min(goals.length / 3, 1) * 10) // 10% on having goals
    );
    ...
    <div>
      ...
      <AnimateIn delay={300}>
        <div className="mt-6 rounded-2xl bg-card p-6 ring-1 ring-border">
          <h2 className="mb-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Badges ({badges.length}/{BADGES.length})</h2>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {BADGES.map((badge) => {
              const earned = badges.includes(badge.key);
              return (
                <div key={badge.key} className={`flex items-center gap-3 rounded-xl p-3 transition ${earned ? "bg-violet-bright/10 ring-1 ring-violet-bright/20" : "bg-surface/40 opacity-50"}`}>
                  <div className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${earned ? "bg-violet-bright/20" : "bg-surface"}`}>
                    <badge.icon className={`h-5 w-5 ${earned ? "text-violet-bright" : "text-muted-foreground"}`} />
                  </div>
                  <div>
                    <p className={`text-sm font-bold ${earned ? "text-foreground" : "text-muted-foreground"}`}>{badge.label}</p>
